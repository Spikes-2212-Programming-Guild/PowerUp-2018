...
public class MoveLift extends CommandGroup {
	...
	public MoveLift(Supplier<Double> speed) {
		...
		addParallel(new MoveBasicSubsystem(Robot.liftLocker,
				() -> SubsystemComponents.Lift.LIMIT_UP.get() ? SubsystemConstants.LiftLocker.LOCK_SPEED.get() : 0.0));
		addSequential(new MoveBasicSubsystemWithTimeSinceReachingLimit(Robot.lift, speed, 0.5));
	}
}
